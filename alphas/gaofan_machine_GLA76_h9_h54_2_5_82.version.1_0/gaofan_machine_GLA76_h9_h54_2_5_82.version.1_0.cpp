...
class GLA76_h9_h54_2_5_82 : public AlphaBase
        {
        public:
                           
explicit GLA76_h9_h54_2_5_82(XMLCONFIG::Element *cfg):
                AlphaBase(cfg), 
                ret(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.s_mfd_inflowvolume_large_order")), 
...
bid_close(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.bid_close")),

mid_high(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.mid_high")),

close_moneyflow_pct_value_l(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.close_moneyflow_pct_value_l")),

turnover_sum(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.turnover_sum")),

bid_open(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.bid_open")),
...
if(bid_close(di-delay,48,ii)!=0)
                {
                 para1[di-delay][ii]=bid_open(di-delay,1,ii)/bid_close(di-delay,48,ii);
                }
...
vector<float> turnover_sum_25_48(24,nan);
                ...
                {
                   para4[di-delay][ii]= QL_Oputils::mean(turnover_sum_25_48_5)/QL_Oputils::mean(turnover_sum_5);
                }
...
criterion[ii] =close_moneyflow_pct_value_l[di-delay][ii];
...
const QL_CUBE<QL_FLOAT> &bid_close;

const QL_CUBE<QL_FLOAT> &mid_high;

const QL_MATRIX<QL_FLOAT> &close_moneyflow_pct_value_l;

const QL_CUBE<QL_FLOAT> &turnover_sum;

const QL_CUBE<QL_FLOAT> &bid_open;
...
AlphaBase * str = new GLA76_h9_h54_2_5_82(cfg);
                        return str;
            }
        }
