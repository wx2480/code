...
class Gavin_Alpha_4th_1 : public AlphaBase
{
    ...
    explicit Gavin_Alpha_4th_1(XMLCONFIG::Element *cfg):
        ...
        Num3(cfg->getAttributeIntDefault("para3",110))
    {
    ...
    void generate(int di) override
    {
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
                ...
                //alpha[ii] =   para_11[di-delay][ii]  * para_7[di-delay][ii]  * para_15[di-delay][ii] * (1- para_17[di-delay][ii]) ;
    ...
    void base_calc(int di)
    {
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            base_alpha[di-delay][ii] =    para_2[di-delay][ii] * para_5[di-delay][ii] *  para_13[di-delay][ii]  ;
    ...
    void daily_calc(int di)
    {
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
           ...
           para_14[di-delay][ii] = s_mfd_inflowvolume[di-delay][ii] ;
            if(bid_mean(di-delay,48,ii)!=0)
            {
                para_15[di-delay][ii] = (ask_mean(di-delay,43,ii) - bid_mean(di-delay,48,ii))/bid_mean(di-delay,48,ii) ;
            }
            para_16[di-delay][ii] = tasm(di-delay,5,ii) - tbsm(di-delay,5,ii) - (tasm(di-delay,1,ii) - tbsm(di-delay,1,ii));
            vector<float> price_allday(24,nan);
            vector<float> vol_allday(24,nan);
            for(int j = 0; j < 12; ++j)
            {
                price_allday[j] = vwap_5min(di-delay,j+1,ii);
                vol_allday[j] = vol_5min(di-delay,j+1,ii);
            }
            for(int j = 12; j < 24; ++j)
            {
                price_allday[j] = vwap_5min(di-delay,j+25,ii);
                vol_allday[j] = vol_5min(di-delay,j+25,ii);
            }   

            para_17[di-delay][ii] = QL_Oputils::corr(price_allday,vol_allday);
