...
namespace
{
class Gavin_Alpha_4th_6 : public AlphaBase
{
    public:
    explicit Gavin_Alpha_4th_6(XMLCONFIG::Element *cfg):
        AlphaBase(cfg), 
    ...
    void base_calc(int di)
    {
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            base_alpha[di-delay][ii] = para_8[di-delay][ii] * para_10[di-delay][ii] * para_11[di-delay][ii];
        }
        weighted_group_rank(di-delay, 70 ,base_alpha[di-delay]);
        return;
    }
    ...
    void daily_calc(int di)
    {
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            ...
            if(sum_volume_price(di-delay,48,ii) != 0)
            {
                para_13[di-delay][ii] = (sum_volume_price(di-delay,43,ii) - sum_volume_price(di-delay,48,ii))/sum_volume_price(di-delay,48,ii);
            }
            para_14[di-delay][ii] = -(tasm(di-delay,45,ii) - tbsm(di-delay,45,ii) - (tasm(di-delay,48,ii) - tbsm(di-delay,48,ii)));
            if(bid_ave_price(di-delay,48,ii) != 0)
            {
                para_15[di-delay][ii] = (bid_ave_price(di-delay,43,ii) - bid_ave_price(di-delay,48,ii))/bid_ave_price(di-delay,48,ii);
            }
            if(bid_mean(di-delay,48,ii)!=0)
            {
                para_16[di-delay][ii] = (bid_mean(di-delay,43,ii) - bid_mean(di-delay,48,ii))/bid_mean(di-delay,48,ii) ;
            }
            if(turnover_mean(di-delay,48,ii)!=0)
            {
                para_17[di-delay][ii] = (turnover_mean(di-delay,43,ii) - turnover_mean(di-delay,48,ii))/turnover_mean(di-delay,48,ii) ;
            }
...
extern "C"
{
    ...
    {
        AlphaBase * str = new Gavin_Alpha_4th_6(cfg);
        return str;
    }
}
