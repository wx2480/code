...
namespace
{
class Gavin_Alpha_4th_2 : public AlphaBase
{
    public:
    explicit Gavin_Alpha_4th_2(XMLCONFIG::Element *cfg):
        AlphaBase(cfg), 
        ...
        sharesout(dr.getData<QL_MATRIX<QL_FLOAT>>("sharesout")), 
        ...
        last_median(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.last_median")),
        ...
        buy_volume_exlarge_order(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.buy_volume_exlarge_order")),
        ...
        net_inflow_rate_volume(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.net_inflow_rate_volume")),
        s_mfd_inflow_closevolume(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.s_mfd_inflow_closevolume")),
        vol_5min(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.vol_sum")),
        vwap_5min(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.sum_volume_price")),
        mid_mean(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.mid_mean")),
        sum_flow(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.sum_flow")),
        Num1(cfg->getAttributeIntDefault("para1",5)),
        Num2(cfg->getAttributeIntDefault("para2",10)),
        Num3(cfg->getAttributeIntDefault("para3",30))
    {
        ...
        para_18.resize(GLOBAL::Dates.size());
        para_19.resize(GLOBAL::Dates.size());
        ...
        for(int di = 0; di < GLOBAL::Dates.size(); ++di)
        {
            ...
            para_18[di].resize(GLOBAL::Instruments.size(), nan);
            para_19[di].resize(GLOBAL::Instruments.size(), nan);
    ...
    void generate(int di) override
    {
        ...
        QL_Oputils::qilin_ar_init(para_18, GLOBAL::Dates.size(), GLOBAL::Instruments.size(), nan);
        QL_Oputils::qilin_ar_init(para_19, GLOBAL::Dates.size(), GLOBAL::Instruments.size(), nan);
    ...
    void base_calc(int di)
    {
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            //base_alpha[di-delay][ii] =    para_18[di-delay][ii] * para_6[di-delay][ii] * para_9[di-delay][ii] * para_2[di-delay][ii]    ;
            base_alpha[di-delay][ii] =    para_5[di-delay][ii] * para_6[di-delay][ii] * para_7[di-delay][ii];
    
        }
        weighted_group_rank(di-delay, 20 ,base_alpha[di-delay]);
        //QL_Oputils::rank(base_alpha[di-delay]);
        return;
    }
    ...
    void daily_calc(int di)
    {
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            ...
            // }
            if(!GLOBALFUNC::equal(bid_ave_price(di-delay,48,ii),0))
            {
                para_13[di-delay][ii] = (ask_ave_price(di-delay,43,ii) - ask_ave_price(di-delay,48,ii))/bid_ave_price(di-delay,48,ii) ;
            }
            std::vector<float> past_last(4,nan);
            std::vector<float> past_flow(4,nan);
            for(int i = 0; i < 4; ++i)
            {
                past_last[i] = last_close(di-delay,43+i,ii);
                past_flow[i] = sum_flow(di-delay,43+i,ii);
            }
            para_14[di-delay][ii] = QL_Oputils::corr(past_last,past_flow);
            if(bid_mean(di-delay,48,ii)!=0)
            {
                para_15[di-delay][ii] = (ask_mean(di-delay,43,ii) - bid_mean(di-delay,48,ii))/bid_mean(di-delay,48,ii) ;
            }
            para_16[di-delay][ii] = s_mfd_inflowvolume[di-delay][ii];
            std::vector<float> past_mid_mean(12,nan);
            std::vector<float> past_volume_mid(12,nan);
            for(int i = 0; i < 4; ++i)
            {
                past_mid_mean[i] = mid_mean(di-delay,43+i,ii);
                past_volume_mid[i] = volume_above_mid(di-delay,43+i,ii);
            }
            para_17[di-delay][ii] = QL_Oputils::corr(past_mid_mean,past_volume_mid);
            std::vector<float> past_s_v_p(6,nan);
            for(int i = 0; i < 6 ; ++i)
            {
                past_s_v_p[i] = sum_volume_price(di-delay,43+i,ii);
            }
            if(QL_Oputils::mean(past_s_v_p)!=0)
            {
                para_18[di-delay][ii] = (sum_volume_price(di-delay,42,ii)-QL_Oputils::mean(past_s_v_p))/QL_Oputils::mean(past_s_v_p);
            }
            if(!GLOBALFUNC::equal(bid_ave_price(di-delay,48,ii),0) && !GLOBALFUNC::equal(bid_ave_price(di-delay,45,ii),0))
            {
                para_19[di-delay][ii] = ask_ave_price(di-delay,45,ii) / bid_ave_price(di-delay,48,ii) -  ask_ave_price(di-delay,42,ii) / bid_ave_price(di-delay,45,ii) ;
            }
        ...
        QL_Oputils::rank(para_18[di-delay]);
        QL_Oputils::rank(para_19[di-delay]);
        return;
    }
    ...
    void weighted_group_rank(int di, int group_total, vector<float>&cross_sectional_vect)
    {
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ii)
        {
            // if(buy_volume_exlarge_order[di-delay][ii]!=0)
            // {
            //     criterion[ii] = buy_volume_small_order[di-delay][ii] / buy_volume_exlarge_order[di-delay][ii] ;
            // }
            criterion[ii] = vwap[di-delay][ii];
        }
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ii)
        {
            if(!GLOBALFUNC::iserr(criterion[ii]))
            {
                ...
                // if(s_mfd_inflowvolume[di-delay][ii] !=0)
                // {
                // weight_standards[group_id][ii] = buy_volume_exlarge_order[di-delay][ii] / s_mfd_inflowvolume[di-delay][ii] ;
                // }
                weight_standards[group_id][ii] = para_16[di-delay][ii];
            }
        }
        ...
        for(int group_id = 0; group_id < group_total; ++ group_id)
        {
            ...
            //group_weight[group_id] = pow(QL_Oputils::hhv(weight_standards[group_id].begin(),weight_standards[group_id].end()),3);
    ...
    private:
        ...
        vector<vector<float>> para_18;
        vector<vector<float>> para_19;
        ...
        const QL_MATRIX<QL_FLOAT> &sharesout; 
        ...
        const QL_CUBE<QL_FLOAT> &last_median;
        ...
        const QL_MATRIX<QL_FLOAT> &buy_volume_exlarge_order;
        const QL_MATRIX<QL_FLOAT> &buy_volume_small_order;
        const QL_MATRIX<QL_FLOAT> &s_mfd_inflowvolume;
        const QL_MATRIX<QL_FLOAT> &net_inflow_rate_volume;
        const QL_MATRIX<QL_FLOAT> &s_mfd_inflow_closevolume;
        const QL_CUBE<QL_FLOAT> &vol_5min;
        const QL_CUBE<QL_FLOAT> &vwap_5min;
        const QL_CUBE<QL_FLOAT> &mid_mean;
        const QL_CUBE<QL_FLOAT> &sum_flow;
        int Num1;
        int Num2;
        int Num3;
};
...
extern "C"
{
    ...
    {
        AlphaBase * str = new Gavin_Alpha_4th_2(cfg);
        return str;
    }
}
