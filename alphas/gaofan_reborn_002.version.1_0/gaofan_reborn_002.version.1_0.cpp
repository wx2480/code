...
ForecastRollRank_con_pb_rank(dr.getData<QL_MATRIX<QL_FLOAT>> ("gogoal2.ForecastRollRank.con_pb_rank")),

adj_open(dr.getData<QL_MATRIX<QL_FLOAT>>("adj_open")),

vwap(dr.getData<QL_MATRIX<QL_FLOAT>>("adj_vwap")),

volume_diff_small_trader(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.volume_diff_small_trader")),
...
s_mfd_inflow(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.s_mfd_inflow")),
            ...
            void generate(int di) override
            {
                ...
                {   
                    if((valid[ di ][ ii ]))
                    {
                        if(rank_alpha2[di-delay-1][ii]!=0)
                        {
                            alpha[ii]=rank_alpha2[di-delay][ii]/rank_alpha2[di-delay-1][ii];
                        }
...
if(adj_close[di-delay][ii] != 0 )
               {
                 para1[di-delay][ii]=(vwap[di-delay][ii]-adj_close[di-delay][ii])/adj_close[di-delay][ii];
               }
...
vector<float> L267_cnt_open_corr(10,nan);
vector<float> L267_cnt_pcorml_corr(10,nan);
for(int i=0;i<10;i++)
{
L267_cnt_open_corr[i]=adj_open[di-delay-i][ii];
L267_cnt_pcorml_corr[i]=volume_diff_small_trader[di-delay-i-1][ii];
}
para4[di-delay][ii]=-(QL_Oputils::corr(L267_cnt_open_corr,L267_cnt_pcorml_corr));


std::vector<float> L107_past_report_37(20,nan);
            std::vector<float> L107_past_eps_37(20,nan);
            for(int i = 0; i < 20 ; i++)
            {
                L107_past_report_37[i] = relative_report_num_10d[di-delay-i][ii];
                L107_past_eps_37[i] = ForecastRollRank_con_pb_rank[di-delay-i][ii];
            }
            para5[di-delay][ii] = QL_Oputils::corr(L107_past_report_37,L107_past_eps_37); //0.2-1 0.1-117 0.24
...
criterion[ii] = s_mfd_inflow[di-delay][ii];
...
const QL_MATRIX<QL_FLOAT> &ForecastRollRank_con_pb_rank;

const QL_MATRIX<QL_FLOAT> &adj_open;

const QL_MATRIX<QL_FLOAT> &vwap;

const QL_MATRIX<QL_FLOAT> &volume_diff_small_trader;
