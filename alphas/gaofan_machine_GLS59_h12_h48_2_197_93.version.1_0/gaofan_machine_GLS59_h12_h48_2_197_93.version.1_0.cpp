...
class GLS59_h12_h48_2_197_93 : public AlphaBase
        {
        public:
                           
explicit GLS59_h12_h48_2_197_93(XMLCONFIG::Element *cfg):
                AlphaBase(cfg), 
...
optimism_conf_75d(dr.getData<QL_CUBE<QL_FLOAT>> ("gogoal2.ConfStk.optimism_conf_75d")),
...
net_inflow_rate_volume_l(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.net_inflow_rate_volume_l")),
...
vector<float> H12_past_vol(5,nan);
                vector<float> H12_past_vwap(5,nan);
                for(int i=0;i<5;i++)
                {
                
                   H12_past_vol[i]= volume[di-delay-i][ii];
                   H12_past_vwap[i]=adj_close[di-delay-i][ii];

                }

               para1[di-delay][ii]=-QL_Oputils::corr(H12_past_vol,H12_past_vwap);

vector<float> H48_cnt_closepan_mean_10(10,nan);
for(int i=0;i<10;i++)
{
H48_cnt_closepan_mean_10[i]=last_high(di-delay,48-i,ii);
}
para2[di-delay][ii]=(-last_high(di-delay,48,ii)+QL_Oputils::mean(H48_cnt_closepan_mean_10));


vector<float> s_mfd_7(10,nan);
                for(int i=0;i<10;i++)
                {
                    s_mfd_7[i]=s_mfd_inflowvolume[di-delay-i][ii];

                }
                para3[di-delay][ii]=1-ts_rank(s_mfd_7);

vector<float> L197_cnt_tsrankmoneyflow_5(5,nan);
 for(int i=0;i<5;i++)
{
L197_cnt_tsrankmoneyflow_5[i]=net_inflow_rate_volume_l[di-delay-i][ii];
}
para4[di-delay][ii]=(1-ts_rank(L197_cnt_tsrankmoneyflow_5));


std::vector<float> L93_past_report_4(60,nan);
            std::vector<float> L93_past_con(60,nan);
            for(int i = 0; i < 60 ; i++)
            {
                L93_past_report_4[i] = relative_report_num_75d[di-delay-i][ii];
                L93_past_con[i] = optimism_conf_75d(di-delay-i,1,ii);
            }
            para5[di-delay][ii] = QL_Oputils::corr(L93_past_report_4,L93_past_con); //0.2-2 0.1-113 1.3    3-6
...
criterion[ii] = s_mfd_inflowvolume[di-delay][ii];
...
const QL_CUBE<QL_FLOAT> &optimism_conf_75d;
...
const QL_MATRIX<QL_FLOAT> &net_inflow_rate_volume_l;
...
AlphaBase * str = new GLS59_h12_h48_2_197_93(cfg);
                        return str;
            }
        }
