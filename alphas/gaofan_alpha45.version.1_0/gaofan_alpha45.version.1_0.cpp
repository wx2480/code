...
namespace
{
class Gavin_Alpha_3rd_3 : public AlphaBase
{
    public:
    explicit Gavin_Alpha_3rd_3(XMLCONFIG::Element *cfg):
        AlphaBase(cfg), 
    ...
    void base_calc(int di)
    {
        ...
        weighted_group_rank(di-delay, 10 ,base_alpha[di-delay]);
        return;
    }
    ...
    void weighted_group_rank(int di, int group_total, vector<float>&cross_sectional_vect)
    {
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ii)
        {
            if(!GLOBALFUNC::iserr(criterion[ii]))
            {
                ...
                weight_standards[group_id][ii] = para_13[di-delay][ii];
            }
        }
...
extern "C"
{
    ...
    {
        AlphaBase * str = new Gavin_Alpha_3rd_3(cfg);
        return str;
    }
}
