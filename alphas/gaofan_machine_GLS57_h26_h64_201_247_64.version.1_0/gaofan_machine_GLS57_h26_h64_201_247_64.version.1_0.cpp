...
class GLS57_h26_h64_201_247_64 : public AlphaBase
        {
        public:
                           
explicit GLS57_h26_h64_201_247_64(XMLCONFIG::Element *cfg):
                AlphaBase(cfg), 
...
ForecastRollRank_con_or_rank(dr.getData<QL_MATRIX<QL_FLOAT>> ("gogoal2.ForecastRollRank.con_or_rank")),

net_inflow_rate_value_l(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.net_inflow_rate_value_l")),

moneyflow_pct_value_l(dr.getData<QL_MATRIX<QL_FLOAT>>("wind.MoneyFlow.moneyflow_pct_value_l")),

bid_median(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.bid_median")),

ave_bid_median(dr.getData<QL_CUBE<QL_FLOAT>>("intervaldata_5min.ave_bid_median")),
...
{
para1[di-delay][ii]=((bid_median(di-delay,6,ii)/bid_median(di-delay,1,ii))-(bid_median(di-delay,48,ii)/bid_median(di-delay,43,ii)));
}

vector<float> H64_cnt_closepan_mean_10(10,nan);
for(int i=0;i<10;i++)
{
H64_cnt_closepan_mean_10[i]=ave_bid_median(di-delay,48-i,ii);
}
para2[di-delay][ii]=(-ave_bid_median(di-delay,48,ii)+QL_Oputils::mean(H64_cnt_closepan_mean_10));


vector<float> L201_cnt_tsrankmoneyflow_5(5,nan);
 for(int i=0;i<5;i++)
{
L201_cnt_tsrankmoneyflow_5[i]=moneyflow_pct_value_l[di-delay-i][ii];
}
para3[di-delay][ii]=(1-ts_rank(L201_cnt_tsrankmoneyflow_5));


vector<float> L247_cnt_inter5_slop(24,nan);
for(int i = 0; i < 6; ++ i){L247_cnt_inter5_slop[i] = last_open(di - delay,i,ii);}
for(int i = 6; i < 12; ++ i){L247_cnt_inter5_slop[i] = last_open(di - delay,i+12,ii);}
for(int i = 12; i < 18; ++ i){L247_cnt_inter5_slop[i] = last_open(di - delay,i+12,ii);}
for(int i = 18; i < 24; ++ i){L247_cnt_inter5_slop[i] = last_open(di - delay,i+24,ii);}
para4[di-delay][ii]=(QL_Oputils::slop(L247_cnt_inter5_slop.begin(),L247_cnt_inter5_slop.end()));


std::vector<float> L64_past_vwap(100,nan);
            std::vector<float> L64_past_market(100,nan);
            for(int i = 0; i < 100 ; i++)
            {
                L64_past_vwap[i] = ForecastRollRank_con_or_rank[di-delay-i][ii];
                L64_past_market[i] = market_conf_15d(di-delay-i,1,ii);
            }
            para5[di-delay][ii] = 1 - QL_Oputils::corr(L64_past_vwap,L64_past_market); //0.2-1 0.1-32 0.39     2-3  1 2 20 4.38
...
const QL_MATRIX<QL_FLOAT> &ForecastRollRank_con_or_rank;

const QL_MATRIX<QL_FLOAT> &net_inflow_rate_value_l;

const QL_MATRIX<QL_FLOAT> &moneyflow_pct_value_l;

const QL_CUBE<QL_FLOAT> &bid_median;

const QL_CUBE<QL_FLOAT> &ave_bid_median;
...
AlphaBase * str = new GLS57_h26_h64_201_247_64(cfg);
                        return str;
            }
        }
